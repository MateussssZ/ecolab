...
static int16_t ECOCALLMETHOD CEcoMemoryManager1Lab_623E1838_Init(/* in */ IEcoMemoryManager1Ptr_t me, /* in */ voidptr_t startAddress, /* in */ uint32_t size) {
    ...
    /* Проверяем что есть достаточно места для хотя бы одного блока */
    if (size < sizeof(EcoMemoryBlock_623E1838) + 16) {
        return -1;
    }

    /* Инициализация кучи */
    pCMe->m_pHeapStart = (char_t*)startAddress;
    pCMe->m_pHeapEnd = pCMe->m_pHeapStart + size;
    pCMe->m_iHeapSize = size;
    
    /* Создание первого блока, занимающего всю кучу */
    pCMe->m_pFirstBlock = (EcoMemoryBlock_623E1838Ptr)startAddress;
    pCMe->m_pFirstBlock->size = size;
    pCMe->m_pFirstBlock->is_free = 1;
    pCMe->m_pFirstBlock->next = NULL;
    pCMe->m_pFirstBlock->prev = NULL;
    
    /* Инициализация свободного списка */
    pCMe->m_pFreeList = pCMe->m_pFirstBlock;
    
    /* Инициализация статистики */
    pCMe->m_iBlockNum = 0;
    pCMe->m_iAllocSize = 0;
    pCMe->m_iTotalAllocated = 0;
    pCMe->m_iTotalFreed = 0;
    pCMe->m_iMaxAllocated = 0;
